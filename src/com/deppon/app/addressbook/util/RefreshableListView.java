...
public class RefreshableListView extends ListView {
	...
	public RefreshableListView(final Context context) {
		super(context);
		System.out.println("RefreshableListView1");
		initialize();
	}

	public RefreshableListView(final Context context, final AttributeSet attrs) {
		super(context, attrs);
		System.out.println("RefreshableListView2");
		initialize();
	}
	...
	public RefreshableListView(final Context context, final AttributeSet attrs,
		...
		System.out.println("RefreshableListView3");
		initialize();
	}

	// 下拉刷新监听
	public void setOnRefreshListener(final OnRefreshListener l) {
		System.out.println("setOnRefreshListener下拉刷新监听");
		mListener = l;
	}

	// 完成刷新
	public void completeRefreshing() {
		System.out.println("completeRefreshing完成刷新");
	...
	public boolean onTouchEvent(final MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			System.out.println("onTouchEvent移动");
			mHistoricalTop = getChildAt(0).getTop();
			break;
		case MotionEvent.ACTION_UP:
			System.out.println("onTouchEvent抬起");
			...
			if (deltaY > config.getScaledTouchSlop()) {
				...
				if (direction > 0) {
					...
					System.out.println("getChildAt(0).getTop()="
							+ getChildAt(0).getTop()+",,height="+height);
					// Refresh bar is extended if top pixel of the first item is
					// visible
					if (getChildAt(0).getTop() == 2) {
						if (mHistoricalTop < 0) {
	...
	private void setHeaderHeight(final int height) {
		System.out.println("setHeaderHeight=" + height);
		if (height <= 1) {
	...
	private void rotateArrow() {
		System.out.println("rotateArrow旋转箭头");
	...
	private void startRefreshing() {
		System.out.println("startRefreshing下拉后刷新数据");
